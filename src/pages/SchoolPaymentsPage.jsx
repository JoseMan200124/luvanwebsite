...
const SchoolPaymentsPage = () => {
    ...
    const fetchAllPayments = async (status = '', q = '') => {
        ...
        try {
            ...
            // Do not reset pagination here; preserve current page.
            // Page resets are handled only when filters/search change.
        } catch (err) {
    ...
    useEffect(() => {
        ...
        try {
            ...
            // Clamp current page if results shrink after refresh
            const total = Array.isArray(arr) ? arr.length : 0;
            const maxPage = Math.max(0, Math.ceil(total / Math.max(1, Number(rowsPerPage || 1))) - 1);
    ...
    // Helper to invalidate all cached payment-history entries for a given paymentId
    const invalidatePaymentHistCacheForPaymentId = useCallback((paymentId) => {
        try {
            if (!paymentId || !regHistCacheRef.current) return;
            const prefix = `${paymentId}:`;
            for (const k of Array.from(regHistCacheRef.current.keys())) {
                if (k.startsWith(prefix)) regHistCacheRef.current.delete(k);
            }
        } catch (e) {
            console.warn('Error invalidating payment history cache for paymentId', paymentId, e);
        }
    }, []);

    // Composite invalidation: some UI caches history by userId, other by paymentId
    const invalidatePaymentHistCache = useCallback((paymentLike) => {
        try {
            const uid = paymentLike?.User?.id || paymentLike?.userId || null;
            const pid = paymentLike?.id || paymentLike?.paymentId || null;
            if (uid) invalidatePaymentHistCacheForUser(uid);
            if (pid) invalidatePaymentHistCacheForPaymentId(pid);
        } catch (e) {
            console.warn('Error invalidating payment history cache', e);
        }
    ...
    const handleConfirmRegister = async () => {
        ...
        try {
            ...
            // Invalidate cached payment histories (some caches are by userId, others by paymentId)
            invalidatePaymentHistCache(registerPaymentTarget);
    ...
    const handleManageAction = async (actionName, payload) => {
        ...
        try {
            ...
            // Invalidate cache for this payment/user so modal will fetch fresh histories
            invalidatePaymentHistCache(payment || manageTarget);
            fetchAllPayments(statusFilter, search);
        } catch (err) {
    ...
    const effectivePendingBalance = Math.max(0, dialogLeftover - dialogCredito);
    const extraDiscountError = dialogExtraDiscount > effectivePendingBalance + 0.005;
    const dialogPaidAmount = Math.max(0, Number(registerAmount || 0));
    const dialogPendingBalance = Math.max(0, dialogLeftover);
    const dialogExtraDiscountAppliedToBalance = Math.min(Math.max(0, dialogExtraDiscount), dialogPendingBalance);
    const dialogGeneratedCredit = Math.max(
        0,
        dialogPaidAmount - Math.max(0, dialogPendingBalance - dialogExtraDiscountAppliedToBalance)
    );
                                ...
                                <Grid container spacing={2}>
                                    ...
                                    <Grid item xs={12} md={5}>
                                        ...
                                        <TextField
                                            ...
                                            disabled={uploadedReceiptsLoading || regHistLoading}
                                            error={extraDiscountError}
                                            helperText={extraDiscountError ? `El descuento no puede exceder el saldo pendiente (Q${effectivePendingBalance.toFixed(2)})` : undefined}
                                            sx={{ mb: 2 }}
                                        />
                                        {dialogGeneratedCredit > 0 && (
                                            <Alert severity="info" sx={{ mb: 2 }}>
                                                Se generará {formatCurrency(dialogGeneratedCredit)} como crédito a favor.
                                            </Alert>
                                        )}
                            ...
                            <DialogActions>
                                ...
                                {paymentTab === 0 && (
                                    <Button variant="contained" onClick={async () => {
                                        ...
                                        setSelectedReceipt(null);
                                        setReceiptZoom(1);
                                    }} disabled={!registerAmount || uploadedReceiptsLoading || regHistLoading || extraDiscountError}>
