...
const formatDate = (d) => {
    if (!d) return '—';
    try { return moment.parseZone(d).format('DD/MM/YYYY'); } catch (e) { return String(d); }
};
...
const OPERATION_META = {
    ...
    EXONERATE_FULL: { icon: <MoneyOffIcon fontSize="small" />, label: 'Exoneración Total', color: '#7b1fa2', bgColor: '#f3e5f5' },
    ...
    DAILY_PENALTY: { icon: <WarningAmberIcon fontSize="small" />, label: 'Mora Diaria', color: '#d32f2f', bgColor: '#ffcdd2' },
...
function getOperationDescription(entry) {
    ...
    switch (op) {
        case 'BILLING': {
            ...
            // El monto facturado es el incremento en el balance (balanceDueAfter - balanceDueBefore).
            // Fallback a metadata si está disponible.
        ...
        case 'PAYMENT': {
            ...
            // Si balanceDueBefore === balanceDueAfter, el pago completo fue sobrepago.
            // Intentar obtener el monto real desde metadata.
            if (amt === 0) {
                amt = Number(meta.amountToBalance || meta.overpayment || meta.overpaymentAmount || 0);
            }
            const txMeta = meta || {};
            const receipt = txMeta.receiptNumber || '';
        ...
        case 'CREDIT_PAYMENT': {
            ...
            const period = meta.period || '';
            return period
                ? `Crédito aplicado a ${formatPeriodLabel(period)} — ${fmt(amt)}`
                : `Uso de crédito disponible — ${fmt(amt)}`;
        }
        case 'OVERPAYMENT': {
            const amt = Number(meta.overpaymentAmount || meta.overpayment || 0);
            return `Sobrepago convertido a crédito — ${fmt(amt)}`;
        }
        case 'REVERSAL':
            return entry.description || meta.reason || 'Reversión de transacción';
        case 'EXONERATION':
        case 'EXONERATE_FULL': {
            const amt = Number(meta.exoneratedAmount || meta.amount || 0);
            if (amt > 0) return `Exoneración de mora — ${fmt(amt)}`;
            return entry.description || 'Exoneración de mora';
        }
        case 'ADJUSTMENT':
        ...
        case 'DAILY_PENALTY':
            return 'Acumulación diaria de mora';
...
const PaymentFlowTimeline = ({ paymentId, userId, familyLastName }) => {
    ...
    const fetchFlow = useCallback(async (page, limit) => {
        ...
        try {
            ...
            setFlowTotal(res.data.pagination?.total || 0);
        } catch (err) {
    ...
    // Ordenar ledger cronológicamente (ascendente para timeline)
    const ledgerAsc = useMemo(() => {
        if (!flowData?.ledger) return [];
        return [...flowData.ledger]
            .filter(e => String(e.operation || '').toUpperCase() !== 'DAILY_PENALTY')
            .sort((a, b) =>
    ...
    // Empty State
    // ============================================================
    if (!flowData || (!payment && ledgerAsc.length === 0)) {
        return (
